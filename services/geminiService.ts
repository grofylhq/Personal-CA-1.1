...
export const transcribeAudio = async (base64Audio: string, mimeType: string): Promise<string> => {
  if (!process.env.API_KEY) throw new Error("API_KEY_NODE_FAULT");
...
export const sendMessageToGemini = async (
  ...
  if (!process.env.API_KEY) throw new Error("CORE_NODE_OFFLINE");
  const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
  ...
  try {
    const responseStream: any = await ai.models.generateContentStream({
      ...
      config: {
        ...
        tools: [{ functionDeclarations: [activateToolDeclaration, draftDocumentDeclaration] }]
      }
    });
      ...
      if (groundingMetadata?.groundingChunks) {
        groundingMetadata.groundingChunks.forEach((c: any) => {
          if (c.web) {
            ...
            if (!sources.find(s => s.uri === source.uri)) sources.push(source);
          }
        });
      }
