...
export const transcribeAudio = async (_base64Audio: string, _mimeType: string): Promise<string> => {
...
export const sendMessageToGemini = async (
  ...
  let data: any;
  try {
    data = await withRetry(() => callOpenRouter(payload));
  } catch (error: any) {
    const message = String(error?.message || '');
    const maybeToolingIssue = message.includes('tool') || message.includes('tools') || message.includes('tool_choice');
    if (!maybeToolingIssue) throw error;
    ...
    data = await withRetry(() => callOpenRouter(fallbackPayload));
  }
